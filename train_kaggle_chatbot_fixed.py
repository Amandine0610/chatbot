#!/usr/bin/env python3
"""
FIXED VERSION - Main training script for healthcare chatbot using Kaggle dataset.
This version has the device error fixed.
"""
...
def main():
    ...
    # Log configuration
    logger.info("=" * 70)
    logger.info("HEALTHCARE CHATBOT TRAINING WITH KAGGLE DATASET (FIXED VERSION)")
    logger.info("=" * 70)
    ...
    try:
        ...
        # Step 3: Model Setup (FIXED - only pass model_key, not output_dir)
        logger.info("Step 3: Model Setup")
        model_manager = HealthcareModelManager(args.model_key)  # FIXED: removed args.output_dir
